...
getFoliosPorMes(anio: number) {
  return this.http.get<any[]>(`http://localhost:8080/api/folios/por-mes/${anio}`);
}

getRankingClientes() {
  return this.http.get<any[]>(`http://localhost:8080/api/folios/ranking-clientes`);
}

getTop10Clientes() {
  return this.http.get<any[]>(`http://localhost:8080/api/folios/ranking-clientes/top10`);
}
